...
bad_urls = []
none_urls = []
base_url = "https://testazure.drivetime.com"
...
def url_checker(url_status):
	try: 
		...
		print("whoops this was an error: {0}".format())
...
rs = (grequests.get(link, timeout=2) for link in main_links)

all_statuses = grequests.map(rs, stream=False, size=100, exception_handler=None, gtimeout=None)
...
if none_urls: 
	for url in none_urls: 
		print("Ummm, this came back wrong, you might want to double check: {0}".format(url))
